      ...
      body: Center(child: SingleChildScrollView(padding:EdgeInsets.all(24), child: Column(children:[
        ...
        Container(padding:EdgeInsets.all(20), decoration:BoxDecoration(color:Colors.white, borderRadius:BorderRadius.circular(20)),
          child: Column(children:[
            Text(isLogin? "Login - Dawo da Level":"Register - Ajiye Level", style:TextStyle(fontWeight:FontWeight.bold, fontSize:18)),
            SizedBox(height:15),
            if(!isLogin) TextField(controller:nameCtrl, decoration:InputDecoration(labelText:"Sunanki", prefixIcon:Icon(Icons.person), border:OutlineInputBorder(borderRadius:BorderRadius.circular(12)))),
            if(!isLogin) SizedBox(height:12),
            TextField(controller:emailCtrl, decoration:InputDecoration(labelText:"Email don Cloud Save", prefixIcon:Icon(Icons.email), border:OutlineInputBorder(borderRadius:BorderRadius.circular(12)))),
            SizedBox(height:15),
            SizedBox(width:double.infinity, child: ElevatedButton(
              ...
              child: Text(isLogin? "Shiga Game 💖":"Yi Register", style:TextStyle(color:Colors.white)),
            )),
...
class _KittyFinalState extends State<KittyFinal> {
  ...
  Future<void> saveToCloud() async {
    ...
    ScaffoldMessenger.of(context).showSnackBar(SnackBar(content:Text("✅ An ajiye a Cloud! Level $roomLevel")));
  }
  ...
  void startTimer(){
    timer=Timer.periodic(Duration(seconds:1),(t){
      if(nextEnergyTime.inSeconds>0){
        setState((){
          nextEnergyTime = nextEnergyTime - Duration(seconds:1);
        });
      } else if(energy<maxEnergy){
        setState((){
          energy++;
          nextEnergyTime=Duration(hours:2);
        });
        saveToCloud();
      }
    });
  ...
  void showEnergy(){
    ...
    showDialog(context:context, builder:(_)=>AlertDialog(
      ...
      actions:[
        ...
        ElevatedButton(onPressed:(){ setState((){ energy=maxEnergy; nextEnergyTime=Duration(hours:2); }); Navigator.pop(context); saveToCloud(); }, child:Text("Tada \$0.99")),
      ],
    ));
  }
  ...
  Widget build(BuildContext context){
    ...
    return Scaffold(
      ...
      appBar:AppBar(backgroundColor:Colors.white, elevation:0,
        title: Row(children:[Text("⚡ $energy/$maxEnergy", style:TextStyle(fontSize:14)), SizedBox(width:6), Text(timeStr, style:TextStyle(fontSize:10))]),
        actions:[Text("💰 $coins"), IconButton(icon:Icon(Icons.cloud, color:Colors.green), onPressed:saveToCloud)],
      ),
      body: Column(children:[
        Container(color:Colors.white, padding:EdgeInsets.all(8), child: Text("👤 $userName | 🏠 Daki $roomLevel/20 - $totalMerges merges")),
        Expanded(child:GridView.builder(padding:EdgeInsets.all(10), gridDelegate:SliverGridDelegateWithFixedCrossAxisCount(crossAxisCount:4, crossAxisSpacing:7, mainAxisSpacing:7), itemCount:20, itemBuilder:(_,i){
          bool empty=board[i]==-1;
          return GestureDetector(onTap:()=>onTapItem(i), child:Container(decoration:BoxDecoration(color:Colors.white, borderRadius:BorderRadius.circular(14)), child:Column(mainAxisAlignment:MainAxisAlignment.center, children:[Text(empty?"➕":mergeChain[board[i]], style:TextStyle(fontSize:28)), Text(empty?"Buy":namesChain[board[i]], style:TextStyle(fontSize:8))])));}
        )),
      ]),
    );
  }
